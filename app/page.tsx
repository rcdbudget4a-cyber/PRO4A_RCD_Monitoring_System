  ...
  const parseFile=async(file:File)=>{
    ...
    try{
      ...
      const preview=official?.map(setDuplicate)??raw.map((row,index)=>{
        const errors:string[]=[];
        if(kind==="claims"){
          const id=readCell(row,["Claim ID","ID"]).toUpperCase();
          const claimType=readCell(row,["Type","Claim Type"]).toUpperCase();
          const date=normalizeDate(readCell(row,["Date of Incident","Incident Date","Date"]));
          const unit=readCell(row,["Unit","Unit / Province","Province"]);
          const year=Number(readCell(row,["Year","CY"])||date.slice(0,4));
          const rawStage=readCell(row,["Workflow Stage","Workflow","Stage"]);
          const record:Claim={id,type:claimType==="KIPO"?"KIPO":"WIPO",year,rank:readCell(row,["Rank"]),name:readCell(row,["Name","Full Name"]).toUpperCase(),province:unit,office:readCell(row,["Office"]),stage:normalizeWorkflow(rawStage),status:normalizeClaimStatus(readCell(row,["Status"]),rawStage),date,dateDisplay:displayDate(date),injury:readCell(row,["Injury","Injury Information"]),sourceCoverage:readCell(row,["Source Coverage","Coverage"]),lastUpdateDate:isoToday()};
          if(!id)errors.push("Missing Claim ID");
          if(!["KIPO","WIPO"].includes(claimType))errors.push("Type must be KIPO or WIPO");
          if(!record.rank)errors.push("Missing rank");
          if(!record.name)errors.push("Missing name");
          if(!date)errors.push("Invalid incident date");
          if(!operationalUnits.includes(unit))errors.push("Invalid unit");
          if(!record.office)errors.push("Missing office");
          if(!record.stage)errors.push("Missing workflow stage");
        ...
        const id=readCell(row,["Retiree ID","ID"]).toUpperCase();
        const retirementDate=normalizeDate(readCell(row,["Date of Retirement","Retirement Date","Date"]));
        const unit=readCell(row,["Unit","Unit / Office","Office"]);
        const year=Number(readCell(row,["Year","CY"])||retirementDate.slice(0,4));
        const calRequirements=readCell(row,["CAL Requirements","CAL"]);
        const record:Retiree={id,year,rank:readCell(row,["Rank"]),name:readCell(row,["Name","Full Name"]).toUpperCase(),unit,retirementDate,retirementDisplay:displayDate(retirementDate),calRequirements,lumpSumRequirements:readCell(row,["Lump Sum Requirements","Lump Sum"]),status:readCell(row,["Status"]),calStatus:/\bcomplete(d)?\b/i.test(calRequirements)?"Processed":"Not Processed",remarks:readCell(row,["Remarks"]),sourceCoverage:readCell(row,["Source Coverage","Coverage"])||`CY ${year}`,lastUpdateDate:isoToday()};
        if(!id)errors.push("Missing Retiree ID");
        if(!record.rank)errors.push("Missing rank");
        if(!record.name)errors.push("Missing name");
        if(!retirementDate)errors.push("Invalid retirement date");
        if(!operationalUnits.includes(unit))errors.push("Invalid unit");
        if(!record.calRequirements)errors.push("Missing CAL requirements");
        if(!record.status)errors.push("Missing status");
        const duplicate=existing.has(id.toLowerCase())||seen.has(id.toLowerCase());
        if(id)seen.add(id.toLowerCase());
        return {row:index+2,id,valid:errors.length===0,duplicate,errors,record};
...
function ArchivePage({profile,notify,recordError}:{profile:UserProfile;notify:(s:string)=>void;recordError:(context:string,error:unknown)=>Promise<void>}) {
  ...
  useEffect(()=>{if(!db)return onSnapshot(collection(db,"archivedRecords"),snapshot=>setItems(snapshot.docs.map(item=>({id:item.id,...item.data()} as ArchivedRecord)).sort((a,b)=>(b.archivedAt?.toDate?.().getTime()||0)-(a.archivedAt?.toDate?.().getTime()||0))),error=>{void recordError("Archive retrieval failed",error);notify("Unable to load archived records.")})},[notify,recordError]);
